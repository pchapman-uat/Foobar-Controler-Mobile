...
import { AlbumSVG, ConnectionSVG, LibrarySVG, PlaybackQueueSVG } from 'managers/SVGManager';
...
class LibraryItem {
  ...
  constructor(name:string, screen: ()=>React.JSX.Element, icon:React.FC<SvgProps>){
    this.name = name;
    this.icon = icon;
    this.screen = screen;
  }
...
export default function LibraryMain() {
    ...
    const Main: React.FC<LibraryProps> = ({setCurrentScreen}) => {
        ...
        <ScrollView style={Styles.Main.containerBlock}>
            {items.map((item, index) => (
              <TouchableOpacity key={index} onPress={() => setCurrentScreen(index)} style={[Styles.Library.itemRow, Styles.Library[index % 2 === 0 ? 'rowEven' : 'rowOdd'] ]}>
                <item.icon style={Styles.Library.icon} width={Styles.Library.icon.width} color={getColor(theme, 'textPrimary')} height={Styles.Library.icon.height}/><Text style={Styles.Library.itemText}>{item.name}</Text>
              </TouchableOpacity>
            ))}
        </ScrollView>
        )
    ...
    return (
      <SafeAreaView style={Styles.Main.container}>
        <Button buttonStyle={Styles.Main.button} title='Back' onPress={() => setCurrentScreen(-1)}/>
